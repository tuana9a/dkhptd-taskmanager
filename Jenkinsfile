            ...
            steps {
                withCredentials([usernamePassword(credentialsId: 'gitlab-username-password-tuana9a', passwordVariable: 'GIT_PASSWORD', usernameVariable: 'GIT_USERNAME')]) {
                    script {
                        ...
                        env.MR_BRANCH_NAME = "bump-dkhptd-taskmanager-version-$BUILD_TAG_SECONDLY"
                        ...
                        sed -i "s|image: tuana9a/dkhptd-taskmanager:.*|image: tuana9a/dkhptd-taskmanager:$BUILD_TAG_SECONDLY|" deploy/dkhptd/k8s/deployments.yaml
                        git add .
                        git commit -m "Bump dkhptd-taskmanager version to $BUILD_TAG_SECONDLY"
