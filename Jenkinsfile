            ...
            steps {
                withCredentials([usernamePassword(credentialsId: 'github-username-password-tuana9a', passwordVariable: 'GIT_PASSWORD', usernameVariable: 'GIT_USERNAME')]) {
                    script {
                        ...
                        sed -i "s|image: tuana9a/dkhptd-taskmanager:.*|image: tuana9a/dkhptd-taskmanager:$BUILD_TAG_SECONDLY|" k8s/deployments.yaml
                        git add .
                        git commit -m "Update dkhptd-taskmanager version to $BUILD_TAG_SECONDLY"
